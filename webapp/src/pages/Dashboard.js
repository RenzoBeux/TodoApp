...
import { getTodos, postTodo, updateTodo } from "../api/todo.api";
import { useSelector, useDispatch } from 'react-redux'
import { setDone, setPage, setTodos, setTotal } from "../store/reducers/todo.reducer";
import { Card, Checkbox, Col, Dropdown, Form, Input, Modal, Pagination, Row, Select } from "antd";
...
export const Dashboard = () => {
    ...
    useEffect(() => {
        getTodos(user.accessToken,0,'','all').then(data => {
            ...
            content: (
              ...
              >
                <Form.Item
                  name="title"
                  label="Title"
                  rules={[{ required: true, message: 'Please input the title of the TODO item!' }]}
                >
                  <Input />
                </Form.Item>
    ...
    return (
        <div>
            ...
            <div className="container">
                <Row gutter={[16, 16]} className="toolbox">
                    <Col xs={24} md={12} span={12}>
                        <Input.Search placeholder="Search" onSearch={value => { 
                            refreshTodos(dispatch, user.accessToken, 0, value, doneFilter);
                        }} enterButton />
                    </Col>
                    <Col xs={24} md={12} span={12}>
                        <Select
                            ...
                            onChange={(value) => { 
                                refreshTodos(dispatch, user.accessToken, 0, search, value);
                ...
                <Row gutter={[16, 16]}>
                    {todos.map(todo => (
                        <Col span={12} key={todo.id}>
                            <Card hoverable title={todo.title} extra={
                ...
                <Pagination className="paginator" current={page +1} defaultCurrent={1} defaultPageSize={ 6 } total={total} onChange={(page) => {
                    getTodos(user.accessToken, page - 1, search, doneFilter).then((data) => { 
                        dispatch(setTodos(data.content));
